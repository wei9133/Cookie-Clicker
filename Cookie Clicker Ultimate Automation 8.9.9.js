...
// @description:zh-TW 全功能自動掛機腳本 v8.9.9 Fortune Cookie Ticker
...
/*
變更日誌 (Changelog):
v8.9.9 Feature & Safety Update (2025):
  - [UI] Settings: 新增設定匯出與匯入功能，方便在不同設備或存檔間轉移設定。
  - [Safety] Store: 強化 Logic.Buy 模組，強制禁止在常規購買循環中購買季節切換器 (Toggle)，防止無限迴圈。
v8.9.8.1 Feature Update (2025):
  - [UI] Remote Control: 新增龍族與聖誕老人面板的遙控切換按鈕 (位於建築列表頂部)。
  - [Logic] Godzamok: 修復買回邏輯，現在會正確遵循建築數量上限 (Building Limit)，防止溢出。
v8.9.7 Safety & UX Update (2025):
  - [Safety] Runtime: 實裝 "Secure by Default" 策略，Runtime.SeasonState.isFarming 預設值改為 true，防止初始化期間的競態條件導致誤買誓約。
  - [UX] Garden Editor: 新增「讀取盤面 (Load Live)」功能，可將當前花園狀態導入編輯器。
  - [UX] Garden Editor: 新增「變異標記」支援，紫色粗體顯示未解鎖植物，並提供「包含變異」過濾選項。
  - [Logic] Season: 優化狀態釋放邏輯，僅在功能關閉或目標達成時顯式釋放打寶狀態。
*/
...
(function() {
                    ...
                    <div style="padding-bottom: 20px; overflow-y: auto; max-height: calc(85vh - 165px);">
                        ...
                        <div id="tab-settings" class="cc-tab-pane">
                            <div class="panel-section" style="margin-bottom:10px; padding:10px; background:#f3e5f5; border-radius:8px;">
                                ...
                                <div style="margin-top:15px; padding-top:10px; border-top:1px dashed #ccc; display:flex; gap:10px;">
                                    <button id="btn-export-settings" style="flex:1; padding:8px; background:#5c6bc0; color:white; border:none; border-radius:4px; cursor:pointer;">📤 匯出設定</button>
                                    <button id="btn-import-settings" style="flex:1; padding:8px; background:#26a69a; color:white; border:none; border-radius:4px; cursor:pointer;">📥 匯入設定</button>
                                </div>
                            </div>
                        </div>
        ...
        bindEvents: function() {
            ...
            $('#btn-export-settings').click(function() {
                try {
                    ...
                    prompt('請複製以下設定代碼 (Base64):', base64Str);
                } catch(e) {
            ...
            $('#btn-import-settings').click(function() {
                const input = prompt('請貼上設定代碼 (Base64):');
                if (!input) return;
                try {
                    ...
                    if (data.Flags) {
                        // Manual mapping for safety and completeness
                        const map = {
                        ...
                        for (let k in data.Flags) {
                            if (map[k]) GM_setValue(map[k], data.Flags[k]);
                        }
                    }
                    ...
                    if (data.Settings) {
                        ...
                        // Direct values
                        if (s.Volume !== undefined) GM_setValue('gameVolume', s.Volume);
                        if (s.ClickInterval !== undefined) GM_setValue('clickInterval', s.ClickInterval);
                        if (s.BuyStrategy !== undefined) GM_setValue('buyStrategy', s.BuyStrategy);
                        if (s.MaxWizardTowers !== undefined) GM_setValue('maxWizardTowers', s.MaxWizardTowers); // Not in UI but in Config

                        // Godzamok
                        if (s.GodzamokMinMult !== undefined) GM_setValue('godzamokMinMult', s.GodzamokMinMult);
                        if (s.GodzamokSellAmount !== undefined) GM_setValue('godzamokSellAmount', s.GodzamokSellAmount);
                        if (s.GodzamokTargetBuilding !== undefined) GM_setValue('godzamokTargetBuilding', s.GodzamokTargetBuilding);
                        if (s.GodzamokBuyback !== undefined) GM_setValue('godzamokBuyback', s.GodzamokBuyback);
                        if (s.GodzamokHotkey !== undefined) GM_setValue('godzamokHotkey', s.GodzamokHotkey);
                        if (s.GodzamokBuyHotkey !== undefined) GM_setValue('godzamokBuyHotkey', s.GodzamokBuyHotkey);
                        if (s.GodzamokRestockAmount !== undefined) GM_setValue('godzamokRestockAmount', s.GodzamokRestockAmount);

                        // Building Limit
                        if (s.BuildingLimit !== undefined) GM_setValue('buildingLimit', s.BuildingLimit);

                        // Time conversions
                        if (s.BuyIntervalMs !== undefined) {
                            const totalSec = Math.floor(s.BuyIntervalMs / 1000);
                            const m = Math.floor(totalSec / 60);
                            const sec = totalSec % 60;
                            GM_setValue('buyIntervalMinutes', m);
                            GM_setValue('buyIntervalSeconds', sec);
                            // Assuming Hours is always 0 based on original script defaulting to minutes/seconds logic usually,
                            // but let's check Config init.
                            // Config uses hours, minutes, seconds.
                            // BuyIntervalMs calculation: (Hr*3600 + Min*60 + Sec) * 1000
                            // To be safe, we can just dump it into Minutes and Seconds if Hours isn't critical, or calculate hours.
                            const h = Math.floor(totalSec / 3600);
                            const remSec = totalSec % 3600;
                            const remMin = Math.floor(remSec / 60);
                            const finalSec = remSec % 60;
                            GM_setValue('buyIntervalHours', h);
                            GM_setValue('buyIntervalMinutes', remMin);
                            GM_setValue('buyIntervalSeconds', finalSec);
                        }

                        if (s.RestartIntervalMs !== undefined) {
                             const totalSec = Math.floor(s.RestartIntervalMs / 1000);
                             const h = Math.floor(totalSec / 3600);
                             const remSec = totalSec % 3600;
                             const m = Math.floor(remSec / 60);
                             const sec = remSec % 60;
                             GM_setValue('restartIntervalHours', h);
                             GM_setValue('restartIntervalMinutes', m);
                             GM_setValue('restartIntervalSeconds', sec);
                        }
                    }

                    alert('設定匯入成功！頁面將重新整理以套用。');
                    location.reload();

                } catch(e) {
                    alert('匯入失敗 (格式錯誤): ' + e.message);
                }
            });
        }
    };
            ...
            update: function(now) {
                ...
                if (Config.Flags.AutoPledge && pledge && pledge.unlocked && pledge.canBuy() && isPledgeExpired) {
                    ...
                    if (Runtime.SeasonState.isFarming) {
                        ...
                        // [v8.9.9 Safety] 季節狀態雙重驗證 (Seasonal Ambiguity Check)
                        // 防止 Logic.Season 因延遲尚未更新 isFarming 狀態時，腳本誤判為"已完成季節"而購買誓約
                        if (Game.season && ['valentines', 'christmas', 'easter', 'halloween'].includes(Game.season)) {
                            // 驗證手段 A: 商店庫存檢查 (最可靠的來源)
                            // 若商店中仍看得到該季節的物品(且非開關)，代表絕對還沒買完。
                            const hasSeasonDrop = Game.UpgradesInStore.some(u => 
                                u.season === Game.season && u.pool !== 'toggle'
                            );
                            if (hasSeasonDrop) {
                                Logger.warn('Buy', `🛑 攔截誓約購買：商店中仍有 ${Game.season} 季節物品未買`);
                                return; // 絕對阻斷
                            }
                            // 驗證手段 B: 數據完整性緩衝
                            // 若商店是空的(長度0)，有可能是剛讀檔還沒 render 出來，給予額外緩衝時間
                            if (Game.UpgradesInStore.length === 0 && 
                                Date.now() < Runtime.Timers.GardenWarmup + 5000) {
                                Logger.warn('Buy', '🛑 攔截誓約購買：商店數據未就緒，延遲檢查');
                                return; // 暫緩
                            }
                        }
                        // 通過驗證後，才允許購買誓約
                ...
                affordable = affordable.filter(u => {
                    ...
                    // [Safety] 季節切換器 (Toggle) 僅允許由 Logic.Season 模組在明確的狀態機控制下購買
    ...
    const Core = {
        ...
        init: function() {
            Logger.success('Core', 'Cookie Clicker Ultimate v8.9.9 Loading...');
