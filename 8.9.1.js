...
// @description:zh-TW 全功能自動掛機腳本 v8.9.1 Fix: Godzamok Race Condition & Remove Legacy Caps
...
v8.9.0 UI & Logic Update (2025):
  - [UI] GardenProtection: 修正花園按鈕精準定位，避免遮擋標題 (top: 50px)。
  - [UI/Logic] Building Cap: 建築限購遷移至「進階」面板，改用下拉選單管理。
  - [Fix] Version Bump to v8.9.0.
v8.8.9.3 UI & Logic Fixes (2025):
  - [UI] GardenProtection: 修正嵌入式控制台佈局，改掛載於 #gardenField 並靠右對齊 (right: 0)。
  - [Feature] Building Cap: 新增建築數量上限設定 (於資金頁籤)，支援自動購買與戰術核彈限制。
  - [Logic] Garden: 優化雜草保護邏輯，僅剷除已解鎖的雜草 (防止誤鏟新品種)。
  - [UI] Styles: 清除部分介面殘留的幽靈粗體文字。
*/
...
(function() {
    ...
    const Logic = {
        ...
        GodzamokTactical: {
            ...
            buyAmmo: function() {
                const targetName = Config.Settings.GodzamokTargetBuilding;
                let amount = Config.Settings.GodzamokSellAmount;
                const obj = Game.Objects[targetName];

                // 檢查：若 obj 存在且在白名單 [2,3,4] 內
                if (obj && [2, 3, 4].includes(obj.id)) {
                    // [v8.8.9.3] Cap Check
                    const limit = (Config.Settings.BuildingLimit && Config.Settings.BuildingLimit[targetName]) ? Config.Settings.BuildingLimit[targetName] : 0;
                    if (limit > 0 && (obj.amount + amount) > limit) {
                        amount = limit - obj.amount;
                        if (amount <= 0) {
                             Logger.warn('Tactical', `快速補貨取消：已達建築上限`);
                             return;
                        }
                    }

                    const cost = obj.getSumPrice(amount);
                    // 且資金足夠
                    if (Game.cookies >= cost) {
                        obj.buy(amount);
                        Logger.log('Tactical', `➕ 快速補貨: ${amount} ${targetName}`);

                        // [Fix v8.8.8.3] 手動介入，解除 F9 鎖定
                        ...
                        // [Safety] 雖然解除了 F9 鎖，但花園必須繼續戒嚴 (重設 60s)
                        // 修復手動補貨導致花園突變無法自動恢復的問題
                        Logic.GodzamokCombo.scheduleRestoration();
                    } else {
    ...
    const Core = {
        ...
        init: function() {
            Logger.success('Core', 'Cookie Clicker Ultimate v8.9.1 Loading...');
