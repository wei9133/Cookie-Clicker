...
// @description:zh-TW 全功能自動掛機腳本 v9.6.1 Oracle Prescience
...
/*
變更日誌 (Changelog):
v9.6.1 Recovery & Oracle V11 (2026):
  - [Critical Fix] 核心: 防呆恢復協議與手動恢復按鈕，綁定強制暴力解鎖，徹底粉碎死鎖。
  - [Logic] 魔法: 升級神諭核心 (Oracle Core V11)，確保全域亂數絕對安全隔離。

v9.6.0 Oracle Prescience (2026):
  - [Feature] 魔法: 新增「神諭預知系統 (Oracle)」單一沙盒雙重推演與快取機制。
  - [UI] 面板: 新增「全域熱鍵矩陣 (Hotkeys Nexus)」統整所有熱鍵並支援雙向綁定。
  - [UI] 魔法: 新增「神諭紅綠燈」功能，可動態改變 Gambler Spin 按鈕顏色。

v9.5.0 The Final Vanguard (2026):
  - [Feature] 設定: 實作「儲存至文件(.txt)」與「從文件讀取(.txt)」功能，支援跨裝置無縫轉移。
  - [Feature] 設定: 實作「恢復出廠預設值」，一鍵清除所有存檔配置與陣型記憶 (受忙碌狀態保護)。
  - [Feature] 戰術核彈: 新增「一鍵連擊 (Macro)」功能與專屬熱鍵防護。
  - [Logic] Godzamok: `buyAmmo` 新增智慧解鎖回歸邏輯，補貨後自動還原原始資金鎖定狀態。
  -[Security] Core: 新增物理防火牆阻斷 AHK 等外部連點程式造成的 Race Condition 與記憶體溢位。

v9.4.0.1 Hotfix (2026):
  -[Critical Fix] 魔法: 修復 Oracle 系統中 PRNG 全域污染問題，改用沙盒隔離實例 (new Math.seedrandom)。

v9.4.0 Arcane Vanguard (2026):
  - [Feature] 魔法: 新增「奧術神諭系統 (Oracle)」，可於戰術面板預測下一次命運之手的結果。
  -[UI] 糖塊: 新增「糖塊紅綠燈 (Lump Radar)」，於遊戲內顯示糖塊成熟倒數狀態，並支援自訂顏色與時間。
...
(function() {
    ...
    const Config = {
        ...
        Flags: {
            ...
            ShowGamblerTrafficLight: GM_getValue('showGamblerTrafficLight', false)
        },
        // 參數
        Settings: {
            ...
            GodzamokMacroHotkey: GM_getValue('godzamokMacroHotkey', 'F11'),
            ...
            GamblerHotkey: GM_getValue('gamblerHotkey', ''),
            HoFHotkey: GM_getValue('hofHotkey', ''),
            ...
            // [v9.4.0] Lump Radar Settings
            LumpTimeRed: GM_getValue('lumpTimeRed', 120),
            LumpTimeYellow: GM_getValue('lumpTimeYellow', 60),
            LumpTimeBlue: GM_getValue('lumpTimeBlue', 30),
            LumpColorRed: GM_getValue('lumpColorRed', '#f44336'),
            LumpColorYellow: GM_getValue('lumpColorYellow', '#ffeb3b'),
            LumpColorBlue: GM_getValue('lumpColorBlue', '#2196f3'),
            LumpColorMature: GM_getValue('lumpColorMature', '#4caf50')
        },
        // 記憶
        Memory: {
            GardenProfiles: null,
...
createControlPanel: function() {
            ...
            this.Elements.Panel = $(`
                ...
                ">
                    ...
                    ">
                        <span>🍪 控制面板 v9.6.1 Oracle Prescience</span>
                    ...
                    <div class="cc-tab-header">
                        ...
                        <button class="cc-tab-btn" data-target="hotkeys">熱鍵</button>
                    ...
                    <div style="padding-bottom: 20px; overflow-y: auto; max-height: calc(85vh - 165px);">
                        ...
                        <div id="tab-advanced" class="cc-tab-pane">
                            <div class="panel-section" style="margin-bottom:15px; padding:10px; background:#e1f5fe; border-radius:8px;">
                                ...
                                <div style="color:#333; font-size:13px; display:grid; gap:8px;">
                                    ...
                                    <label><input type="checkbox" class="chk-magic-monitor" ${Config.Flags.ShowMagicMonitor?'checked':''}> 🔮 奧術戰術面板</label>
                                    <label style="margin-left: 20px; font-size: 12px; color: #ffeb3b; display: block;">
                                        └ <input type="checkbox" class="chk-gambler-traffic-light" ${Config.Flags.ShowGamblerTrafficLight?'checked':''}> 🚦 啟用神諭紅綠燈
                                        <div style="font-size:10px; color:#aaa; margin-top:2px;">⚠️ 改變 Gambler 按鈕顏色</div>
                                    </label>
                                </div>
                                <div style="margin-top:10px; padding-top:10px; border-top:1px dashed #bcd; display:block;">
                                    ...
                                    <div style="font-size:11px; color:#555; margin-left:20px; margin-top:6px; padding-top:6px; border-top:1px dotted #ccc;">
                                        ...
                                        觸發熱鍵: <input type="text" class="sync-hk-f9 cc-hk-input" value="${Config.Settings.GodzamokHotkey}" style="width:60px;"> (修飾鍵: ^Ctrl, +Shift, !Alt, #Win)<br>
                                        補貨熱鍵: <input type="text" class="sync-hk-f10 cc-hk-input" value="${Config.Settings.GodzamokBuyHotkey}" style="width:60px;"> (預設 F10)<br>
                                        一鍵連擊: <input type="text" class="sync-hk-f11 cc-hk-input" value="${Config.Settings.GodzamokMacroHotkey}" style="width:60px;">
                                        <div style="font-size:10px; color:#ff5252; margin-top:4px; border-top:1px dotted #ccc; padding-top:4px;">⚠️ 警告：一鍵連擊為大後期專用。若使用 AHK 等外部程式，請將按鍵發送間隔 (Delay) 設為 100ms 以上，避免瀏覽器與遊戲腳本崩潰！</div>
                                    </div>
                                </div>
                                ...
                                <div class="cc-garden-panel-standard" style="margin-top:10px; border: 1px solid #90caf9; background: #e3f2fd !important;">
                                    ...
                                    <div style="display:flex; align-items:center; margin-bottom:5px; justify-content:space-between;">
                                        ...
                                        <div style="font-size:11px;">
                                            熱鍵: <input type="text" class="sync-hk-f2 cc-hk-input" value="${Config.Settings.LoanHotkey}" style="width:40px; text-align:center;" title="觸發熱鍵 (預設 F2)">
                                        </div>
                                    </div>
                        ...
                        <div id="tab-garden" class="cc-tab-pane">
                            <div class="cc-garden-panel-standard">
                                ...
                                <div style="display:flex; align-items:center; margin-bottom:8px; gap:5px;">
                                    ...
                                    <input type="text" class="sync-hk-f4 cc-hk-input" value="${Config.Settings.ForceMutationHotkey}"
                        ...
                        <!-- Hotkeys Nexus -->
                        <div id="tab-hotkeys" class="cc-tab-pane">
                            <div class="panel-section" style="margin-bottom:15px; padding:10px; background:#f0f4c3; border-radius:8px;">
                                <div style="font-weight:bold; color:#827717; margin-bottom:10px; border-bottom:1px solid #cddc39; padding-bottom:5px;">⌨️ 全域熱鍵矩陣 (Hotkeys Nexus)</div>
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; font-size:12px; color:#333;">
                                    <div><label>銀行貸款 (2)</label><br><input type="text" class="sync-hk-f2 cc-hk-input" value="${Config.Settings.LoanHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>強制變異 (花園)</label><br><input type="text" class="sync-hk-f4 cc-hk-input" value="${Config.Settings.ForceMutationHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>Godzamok 全賣</label><br><input type="text" class="sync-hk-f9 cc-hk-input" value="${Config.Settings.GodzamokHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>Godzamok 補貨</label><br><input type="text" class="sync-hk-f10 cc-hk-input" value="${Config.Settings.GodzamokBuyHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>一鍵連擊 (Macro)</label><br><input type="text" class="sync-hk-f11 cc-hk-input" value="${Config.Settings.GodzamokMacroHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>Gambler Spin</label><br><input type="text" class="sync-hk-gambler cc-hk-input" value="${Config.Settings.GamblerHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                    <div><label>命運之手</label><br><input type="text" class="sync-hk-hof cc-hk-input" value="${Config.Settings.HoFHotkey}" style="width:100%; max-width:120px; text-align:center; margin-top:4px;"></div>
                                </div>
                                <div style="font-size:11px; color:#555; margin-top:10px; padding-top:5px; border-top:1px dashed #cddc39;">💡 提示：在此處修改的熱鍵，將自動同步至所有對應的模組設定中。允許使用修飾鍵 (^Ctrl, +Shift, !Alt, #Win)。輸入留空可停用該熱鍵。</div>
                            </div>
                        </div>
                        ...
                        <div id="tab-settings" class="cc-tab-pane">
                            ...
                            <!--[v9.4.0] Lump Radar Settings -->
                            <div class="panel-section" style="margin-bottom:10px; padding:10px; background:#fce4ec; border-radius:8px;">
                                <div style="font-weight:bold; color:#880e4f; margin-bottom:8px; border-bottom:1px solid #f8bbd0; padding-bottom:4px;">🍬 糖塊雷達設定</div>
                                <div style="display:flex; flex-direction:column; gap:6px; font-size:12px; color:#333;">
                                    <div style="display:flex; justify-content:space-between; align-items:center;">
                                        <label>🔴 紅燈時間 (分鐘):</label>
                                        <input type="number" id="val-lump-red" value="${Config.Settings.LumpTimeRed}" style="width:50px; text-align:center;">
                                    </div>
                                    <div style="display:flex; justify-content:space-between; align-items:center;">
                                        <label>🟡 黃燈時間 (分鐘):</label>
                                        <input type="number" id="val-lump-yellow" value="${Config.Settings.LumpTimeYellow}" style="width:50px; text-align:center;">
                                    </div>
                                    <div style="display:flex; justify-content:space-between; align-items:center;">
                                        <label>🔵 藍燈時間 (分鐘):</label>
                                        <input type="number" id="val-lump-blue" value="${Config.Settings.LumpTimeBlue}" style="width:50px; text-align:center;">
                                    </div>
                                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px; margin-top:5px; border-top:1px dashed #f8bbd0; padding-top:5px;">
                                        <div style="display:flex; flex-direction:column; gap:2px;">
                                            <label>🔴 紅燈:</label>
                                            <div style="display:flex; gap:3px;">
                                                <input type="color" id="color-lump-red" value="${Config.Settings.LumpColorRed}" style="width:28px; height:24px; padding:0; border:1px solid #ccc; cursor:pointer;">
                                                <input type="text" id="text-lump-red" value="${Config.Settings.LumpColorRed}" style="width:60px; font-family:monospace; font-size:11px; text-transform:uppercase; text-align:center;">
                                            </div>
                                        </div>
                                        <div style="display:flex; flex-direction:column; gap:2px;">
                                            <label>🟡 黃燈:</label>
                                            <div style="display:flex; gap:3px;">
                                                <input type="color" id="color-lump-yellow" value="${Config.Settings.LumpColorYellow}" style="width:28px; height:24px; padding:0; border:1px solid #ccc; cursor:pointer;">
                                                <input type="text" id="text-lump-yellow" value="${Config.Settings.LumpColorYellow}" style="width:60px; font-family:monospace; font-size:11px; text-transform:uppercase; text-align:center;">
                                            </div>
                                        </div>
                                        <div style="display:flex; flex-direction:column; gap:2px;">
                                            <label>🔵 藍燈:</label>
                                            <div style="display:flex; gap:3px;">
                                                <input type="color" id="color-lump-blue" value="${Config.Settings.LumpColorBlue}" style="width:28px; height:24px; padding:0; border:1px solid #ccc; cursor:pointer;">
                                                <input type="text" id="text-lump-blue" value="${Config.Settings.LumpColorBlue}" style="width:60px; font-family:monospace; font-size:11px; text-transform:uppercase; text-align:center;">
                                            </div>
                                        </div>
                                        <div style="display:flex; flex-direction:column; gap:2px;">
                                            <label>🟢 成熟:</label>
                                            <div style="display:flex; gap:3px;">
                                                <input type="color" id="color-lump-mature" value="${Config.Settings.LumpColorMature}" style="width:28px; height:24px; padding:0; border:1px solid #ccc; cursor:pointer;">
                                                <input type="text" id="text-lump-mature" value="${Config.Settings.LumpColorMature}" style="width:60px; font-family:monospace; font-size:11px; text-transform:uppercase; text-align:center;">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="panel-section" style="margin-bottom:10px; padding:10px; background:#f3e5f5; border-radius:8px;">
                                ...
                                <label style="display:block; font-size:13px; color:#333;"><input type="checkbox" class="chk-magic-monitor" ${Config.Flags.ShowMagicMonitor?'checked':''}> 🔮 奧術戰術面板</label>
                                ...
                                <div style="margin-top:10px; border-top:1px solid #ccc; padding-top:8px;">
                                    ...
                                    <div style="display:flex; flex-direction:column; gap:5px;">
                                        <div style="display:flex; gap:5px;">
                                            <button id="btn-settings-export" style="flex:1; padding:5px; background:#5c6bc0; color:white; border:none; border-radius:4px; cursor:pointer;">📤 匯出設定(代碼)</button>
                                            <button id="btn-settings-import" style="flex:1; padding:5px; background:#ef5350; color:white; border:none; border-radius:4px; cursor:pointer;">📥 匯入設定(代碼)</button>
                                        </div>
                                        <div style="display:flex; gap:5px;">
                                            <button id="btn-settings-export-file" style="flex:1; padding:5px; background:#00838f; color:white; border:none; border-radius:4px; cursor:pointer;">📄 儲存至文件(.txt)</button>
                                            <button id="btn-settings-import-file" style="flex:1; padding:5px; background:#00695c; color:white; border:none; border-radius:4px; cursor:pointer;">📂 從文件讀取(.txt)</button>
                                        </div>
                                        <div style="display:flex; gap:5px;">
                                            <button id="btn-settings-factory-reset" style="flex:1; padding:5px; background:#424242; color:#ff7043; font-weight:bold; border:none; border-radius:4px; cursor:pointer; transition: background 0.2s;" onmouseover="this.style.background='#ff5252'; this.style.color='#fff';" onmouseout="this.style.background='#424242'; this.style.color='#ff7043';">⚠️ 恢復出廠預設值</button>
                                        </div>
                                    </div>
                                </div>
        ...
        bindEvents: function() {
            ...
            $('.chk-magic-monitor').change(function() {
                ...
                Config.Flags.ShowMagicMonitor = isChecked;
                GM_setValue('showMagicMonitor', isChecked);
                
                // 強制同步兩個分頁的 Checkbox 視覺狀態
                $('.chk-magic-monitor').prop('checked', isChecked); 
                
                if (UI.MagicMonitor) UI.MagicMonitor.toggle(isChecked);
            });

            // 神諭紅綠燈開關
            $('.chk-gambler-traffic-light').change(function() {
                const isChecked = this.checked;
                Config.Flags.ShowGamblerTrafficLight = isChecked;
                GM_setValue('showGamblerTrafficLight', isChecked);
                
                // 強制同步所有對應 class 的勾選框
                $('.chk-gambler-traffic-light').prop('checked', isChecked);
                
                // 觸發重新渲染
                if (UI.MagicMonitor && UI.MagicMonitor.update) UI.MagicMonitor.update();
            });
            ...
            // [v9.6.0] 統一全域雙向熱鍵綁定 (Bi-directional Sync)
            const bindSyncHotkey = (cls, configKey, gmKey) => {
                $(document).on('change input', '.' + cls, function() {
                    let val = $(this).val().trim(); // 允許空字串儲存，作為停用熱鍵
                    $('.' + cls).val(val);
            ...
            bindSyncHotkey('sync-hk-f9', 'GodzamokHotkey', 'godzamokHotkey');
            bindSyncHotkey('sync-hk-f10', 'GodzamokBuyHotkey', 'godzamokBuyHotkey');
            bindSyncHotkey('sync-hk-f11', 'GodzamokMacroHotkey', 'godzamokMacroHotkey');
            bindSyncHotkey('sync-hk-f4', 'ForceMutationHotkey', 'forceMutationHotkey');
            bindSyncHotkey('sync-hk-f2', 'LoanHotkey', 'loanHotkey');
            bindSyncHotkey('sync-hk-gambler', 'GamblerHotkey', 'gamblerHotkey');
            bindSyncHotkey('sync-hk-hof', 'HoFHotkey', 'hofHotkey');
            ...
            const bindColorSync = (id, configKey, gmKey) => {
                ...
                $(`#color-${id}`).on('input', function() {
                    ...
                    Config.Settings[configKey] = val; // 更新運行記憶體 (大寫開頭)
                    GM_setValue(gmKey, val);          // 寫入油猴存檔 (小寫開頭)
                    if (UI.GardenProtection && UI.GardenProtection.updateEmbeddedState) {
            ...
            //[v9.4.0] Lump Radar Settings
            $('#val-lump-red').change(function() {
                let val = parseInt(this.value);
                if (isNaN(val) || val < 0) val = 120;
                Config.Settings.LumpTimeRed = val;
                this.value = val;
                GM_setValue('lumpTimeRed', val);
            });
            $('#val-lump-yellow').change(function() {
                let val = parseInt(this.value);
                if (isNaN(val) || val < 0) val = 60;
                Config.Settings.LumpTimeYellow = val;
                this.value = val;
                GM_setValue('lumpTimeYellow', val);
            });
            $('#val-lump-blue').change(function() {
                let val = parseInt(this.value);
                if (isNaN(val) || val < 0) val = 30;
                Config.Settings.LumpTimeBlue = val;
                this.value = val;
                GM_setValue('lumpTimeBlue', val);
            });

            bindColorSync('lump-red', 'LumpColorRed', 'lumpColorRed');
            bindColorSync('lump-yellow', 'LumpColorYellow', 'lumpColorYellow');
            bindColorSync('lump-blue', 'LumpColorBlue', 'lumpColorBlue');
            bindColorSync('lump-mature', 'LumpColorMature', 'lumpColorMature');
            ...
            $('#btn-force-recover').click(() => { 
                if(confirm('確定立刻執行防呆恢復 (開啟魔法與強制解鎖花園變異)？')) {
                    ...
                    Logic.EmergencyForceMutation(); // 暴力解鎖與換土
                    Logger.success('Recovery', '已手動執行防呆恢復 (魔法與花園皆已強制重啟)！');
                } 
            });

            //[v9.4.0.1 Unicode Fix] Settings Export Handler
            $('#btn-settings-export').click(() => {
                try {
                    ...
                    // 現代化 Unicode 轉 Base64 處理 (TextEncoder)
                    ...
                    // 安全備份到控制台，防止瀏覽器 prompt 截斷長字串
                    console.log("=== CookieBot 完整設定代碼 ===");
                    console.log(b64Str);
                    console.log("==============================");
                    
                    prompt('請複製以下代碼保存（若太長請至控制台 F12 複製）：', b64Str);
                } catch(e) { 
            ...
            $('#btn-settings-import').click(() => {
                let input = prompt('請貼上設定代碼：');
                if(!input) return;
                
                try {
                    // 🚨 核心加固：移除所有換行、空格、非法字元，防止 atob 崩潰
                    const cleanedInput = input.replace(/[^A-Za-z0-9+/=]/g, "");
                    
                    // 現代化 Base64 轉 Unicode 處理 (TextDecoder)
                    ...
                    if(data.Flags) {
                        const flagMap = {
                             ...
                             AutoRestart: 'isAutoRestartEnabled',
                             ShowGamblerTrafficLight: 'showGamblerTrafficLight'
                        };
                    ...
                    if(data.Settings) {
                        const settingMap = {
                            ...
                            GodzamokMacroHotkey: 'godzamokMacroHotkey',
                            ...
                            ColorSniperActive: 'colorSniperActive',
                            ColorSniperInactive: 'colorSniperInactive',
                            LumpTimeRed: 'lumpTimeRed',
                            LumpTimeYellow: 'lumpTimeYellow',
                            LumpTimeBlue: 'lumpTimeBlue',
                            LumpColorRed: 'lumpColorRed',
                            LumpColorYellow: 'lumpColorYellow',
                            LumpColorBlue: 'lumpColorBlue',
                            LumpColorMature: 'lumpColorMature'
                        };
                    ...
                    alert('匯入失敗：代碼格式錯誤或不完整。\n(建議從原始頁面的控制台 F12 複製代碼)'); 
                }
            });

            //[v9.5.0] Settings Management: File Export, Import, Factory Reset
            $('#btn-settings-export-file').click(() => {
                try {
                    const data = {
                        Flags: Config.Flags,
                        Settings: Config.Settings,
                    ...
                    const blob = new Blob([b64Str], { type: 'text/plain' });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    const date = new Date();
                    const dateString = date.getFullYear() + '-' + String(date.getMonth() + 1).padStart(2, '0') + '-' + String(date.getDate()).padStart(2, '0');
                    a.download = `CookieBot_Config_${dateString}.txt`;
                    ...
                    alert('匯出文件失敗: ' + e.message);
                }
            });

            $('#btn-settings-import-file').click(() => {
                ...
                input.onchange = e => {
                    ...
                    reader.onload = function(event) {
                        try {
                            ...
                            const cleanedInput = fileContent.replace(/[^A-Za-z0-9+/=]/g, "");
                            ...
                            if(data.Flags) {
                                ...
                                for(let k in data.Flags) {
                                    if (flagMap[k]) GM_setValue(flagMap[k], data.Flags[k]);
                                }
                            }
                            ...
                            if(data.Settings) {
                                ...
                                for(let k in data.Settings) {
                                    if (settingMap[k]) GM_setValue(settingMap[k], data.Settings[k]);
                                }
                                if (typeof data.Settings.BuyIntervalMs !== 'undefined') {
                                    const ms = data.Settings.BuyIntervalMs;
                                    const mins = Math.floor(ms / 60000);
                            ...
                            alert('從文件匯入成功！頁面將重新載入以套用設定。');
                            location.reload();
                        } catch(err) {
                            console.error("文件解碼失敗:", err);
                            alert('從文件匯入失敗：代碼格式錯誤或不完整。');
                        } finally {
                            input.remove();
                        }
                    };
            ...
            $('#btn-settings-factory-reset').click(() => {
                if (Logic.checkBusyState()) {
                    alert('⛔ 系統忙碌中 (Godzamok/Combat/Sniper 活躍中)。為防止存檔損毀，拒絕執行出廠重置。');
                    return;
                }
                
                if (confirm("⚠️ 警告：這將清除所有 CookieBot 設定、陣型記憶與狙擊手數據，並完全恢復出廠預設值！\n此操作無法復原，確定要繼續嗎？")) {
                    try {
                        GM_listValues().forEach(key => GM_deleteValue(key));
                        alert('✅ 已清除所有 CookieBot 設定。頁面將重新載入。');
                        location.reload();
                    } catch(e) {
                        alert('出廠重置失敗: ' + e.message);
                    }
                }
            });
...
// ═══════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════
    //[v9.4.0 / 9.6.0] 奧術戰術面板 (Grimoire Tactical HUD) 與 神諭系統
    // ═══════════════════════════════════════════════════════════════
    UI.MagicMonitor = {
        Elements: { Container: null, Stats: null, BtnGambler: null, BtnHoF: null, BtnCBG: null, Oracle0: null, Oracle1: null },
        create: function() {
            if (this.Elements.Container) return;
            this.Elements.Container = $(`
                ...
                ">
                    ...
                    <div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px dashed #7b1fa2;">
                        <label style="display:flex; align-items:center; cursor:pointer; font-size:12px; color:#ffcc80;" title="勾選後將關閉自動魔法連擊與閒置召喚，保護下一次施法">
                            <input type="checkbox" id="chk-magic-freeze" style="margin-right:4px;">
                            ⛔ 凍結自動施法 (鎖定法術)
                        </label>
                    </div>
                    <div style="font-size:12px; margin-bottom:8px; padding-bottom:8px; border-bottom:1px solid #7b1fa2; color:#ccc;">
                        👁️ 下一次命運之手預測：<br>
                        <span style="color:#4caf50;">🟢 (無金餅乾): </span><span id="oracle-0" style="font-weight:bold; color:#fff;">讀取中...</span><br>
                        <span style="color:#ffeb3b;">🟡 (有金餅乾): </span><span id="oracle-1" style="font-weight:bold; color:#fff;">讀取中...</span>
                    </div>
                    <div style="display:flex; flex-direction: column; gap: 6px;">
                        <button id="btn-magic-gambler" style="padding: 6px; background: #4a148c; color: white; border: 1px solid #7b1fa2; border-radius: 4px; cursor: pointer; font-weight: bold; transition: all 0.3s ease;">🎰 Gambler Spin</button>
            ...
            this.Elements.Oracle0 = this.Elements.Container.find('#oracle-0');
            this.Elements.Oracle1 = this.Elements.Container.find('#oracle-1');
            
            this.Elements.Container.find('#magic-monitor-close').click(() => {
                $('.chk-magic-monitor').prop('checked', false).trigger('change');
            });

            const freezeChk = this.Elements.Container.find('#chk-magic-freeze');
            // 初始化狀態
            freezeChk.prop('checked', !(Config.Flags.Spell || Config.Flags.SE));
            // 綁定點擊事件
            freezeChk.change(function() {
                const isFrozen = $(this).is(':checked');
                // 反向連動主面板的開關
                $('#chk-spell').prop('checked', !isFrozen).trigger('change');
                $('#chk-se').prop('checked', !isFrozen).trigger('change');
                if (isFrozen) {
                    Logger.warn('Magic', '已手動凍結自動施法，保護當前法術佇列。');
                } else {
                    Logger.success('Magic', '已解除魔法凍結，恢復自動施法。');
                }
            });
        ...
        update: function() {
            ...
            //[v9.6.0] 呼叫 Oracle 更新預測 (內部會自動走 Cache 判斷)
            if (Logic.Oracle && Logic.Oracle.updatePredictions) {
                Logic.Oracle.updatePredictions();
                
                const cache = Logic.Oracle._cache;
                
                const formatPrediction = (result) => {
                    if (result.includes("Click Frenzy") || result.includes("Building Special") || result.includes("點擊狂暴") || result.includes("建築增益")) {
                        return `<span style="color:#00ffff; text-shadow:0 0 5px #00ffff;">${result}</span>`;
                    }
                    return result;
                };

                this.Elements.Oracle0.html(formatPrediction(cache.fthof0));
                this.Elements.Oracle1.html(formatPrediction(cache.fthof1));
                
                //[v9.6.0] 神諭紅綠燈：動態改變 Gambler 按鈕顏色
                if (Config.Flags.ShowGamblerTrafficLight) {
                    let bg = '#4a148c', border = '#7b1fa2'; // Default Purple
                    if (cache.gamblerColor === 'Green') {
                        bg = '#1b5e20'; border = '#4caf50';
                    } else if (cache.gamblerColor === 'Yellow') {
                        bg = '#f57f17'; border = '#ffeb3b';
                    } else if (cache.gamblerColor === 'Red') {
                        bg = '#b71c1c'; border = '#ff5252';
                    }
                    
                    this.Elements.BtnGambler.css({
                        'background': bg,
                        'border-color': border,
                        'box-shadow': cache.gamblerColor !== 'Purple' ? `0 0 10px ${border}` : 'none'
                    });
                } else {
                    this.Elements.BtnGambler.css({
                        'background': '#4a148c',
    ...
    // ═══════════════════════════════════════════════════════════════
    // 2. 核心邏輯模組 (Logic Fix v9.2.1 Matrix Sniper + v9.6.0 Oracle)
    // ═══════════════════════════════════════════════════════════════
    const Logic = {
        //[v9.1.3] 系統狀態檢查
		...
		// ═══════════════════════════════════════════════════════════════
        //[v9.6.0] 奧術神諭系統 (Oracle) 狀態快取與單一沙盒隔離 (mylaaan V11 核心)
        // ═══════════════════════════════════════════════════════════════
        Oracle: {
            _cache: { key: '', fthof0: '', fthof1: '', gamblerColor: 'Purple' },
            ...
            updatePredictions: function() {
                ...
                let auraMult = (typeof Game.auraMult === 'function') ? Game.auraMult('Supreme Intellect') : 0;
                let shimmerCount = Game.shimmers.length;
                let buffAdept = Game.hasBuff('Magic adept') ? 1 : 0;
                let buffInept = Game.hasBuff('Magic inept') ? 1 : 0;
                let buffDiminish = Game.hasBuff('Diminish ineptitude') ? 1 : 0;
                
                // [更新] 將 Game.chimeType 與 Game.ascensionMode 加入快取 Key，因為它們會影響 RNG 消耗
                const currentKey =[Game.seed, M.spellsCastTotal, shimmerCount, buffAdept, buffInept, buffDiminish, auraMult, Game.elderWrath, Game.season, Game.chimeType, Game.ascensionMode].join('|');
                
                if (currentKey === this._cache.key) return;

                const originalRandom = Math.random;

                try {
                    // --- 完美還原 mylaaan / FtHoF-Planner-v4 演算法 ---
                    const getPrediction = (simulatedShimmerCount) => {
                        // 1. 計算真實的 failChance
                        let failChance = 0.15;
                        if (buffAdept) failChance *= 0.1;
                        if (buffInept) failChance *= 5;
                        if (buffDiminish) failChance *= 0.1;
                        failChance += 0.15 * simulatedShimmerCount;
                        if (auraMult > 0) failChance += 0.1 * auraMult;
                        failChance = Math.max(failChance, 0);

                        // 2. 鎖定亂數種子
                        Math.seedrandom(Game.seed + '/' + M.spellsCastTotal);

                        // 3. 成功/逆火 判定 (完全遵守 mylaaan: < 1 - failChance)
                        let isSuccess = Math.random() < (1 - failChance);
                        let isBackfire = !isSuccess;

                        // 4. X 和 Y 座標 (絕對不能省略，必定消耗 2 次亂數)
                        Math.random(); 
                        Math.random(); 

                        // 5. 季節判定 (Valentines 或 Easter 會額外消耗 1 次)
                        // (FtHoF 召喚會帶入 noWrath:true，不會有阿嬤末日的亂數判定)
                        if (Game.season === 'valentines' || Game.season === 'easter') {
                            Math.random();
                        }

                        // 6. 提示音判定 (Chime)
                        // 原版邏輯: 開啟提示音 + 不是出生模式 + 場上目前沒有任何金餅乾 = 消耗 1 次亂數
                        if (Game.chimeType === 1 && Game.ascensionMode !== 1 && simulatedShimmerCount === 0) {
                            Math.random();
                        }

                        // 7. 構建獎池 (完全對齊 mylaaan 源碼小寫格式)
                        let choices =[];
                        if (isSuccess) {
                            ...
                            if (Math.random() < 0.1) choices.push('cursed finger', 'blood frenzy'); // blood frenzy 即 Elder Frenzy
                        ...
                        // 9. 映射回原版 UI 字典格式 (對齊源碼全小寫)
                        const dict = {
                            'frenzy':           { en: 'Frenzy',           zh: '狂暴',       mult: 'x7' },
                            'multiply cookies': { en: 'Lucky',            zh: '幸運',       mult: '+🍪' },
                            'click frenzy':     { en: 'Click Frenzy',     zh: '點擊狂暴',   mult: 'x777' },
                            'cookie storm':     { en: 'Cookie Storm',     zh: '餅乾風暴',   mult: 'Storm' },
                            'cookie storm drop':{ en: 'Storm Drop',       zh: '風暴水滴',   mult: '+🍪' },
                            'cookie chain':     { en: 'Cookie Chain',     zh: '餅乾鏈',     mult: 'Chain' },
                            'blab':             { en: 'Blab',             zh: '廢話',       mult: '無效' },
                            'building special': { en: 'Building Special', zh: '建築增益',   mult: 'Bldg' },
                            'free sugar lump':  { en: 'Sweet!',           zh: '免費糖塊',   mult: '+🍬' },
                            'ruin cookies':     { en: 'Ruin',             zh: '毀滅',       mult: '-🍪' },
                            'clot':             { en: 'Clot',             zh: '凝塊',       mult: 'x0.5' },
                            'cursed finger':    { en: 'Cursed Finger',    zh: '詛咒手指',   mult: 'Click' },
                            'blood frenzy':     { en: 'Elder Frenzy',     zh: '長者狂暴',   mult: 'x666' }
                        };
                        
                        let isZH = false;
                        let testStr = ($('#storeTitle').text() || '') + (Game.Objects['Cursor'] ? (Game.Objects['Cursor'].single || '') : '');
                        if (testStr.match(/[\u4e00-\u9fa5]/) || (typeof Game.language !== 'undefined' && Game.language.includes('ZH'))) {
                            isZH = true;
                        }
                        
                        let info = dict[result] || { en: result, zh: result, mult: '' };
                        let baseName = isZH ? info.zh : info.en;
                        let displayMult = info.mult;
                        if (!isZH && displayMult === '無效') displayMult = 'None';
                        
                        let finalResult = `${baseName} (${displayMult})`;
                        if (isBackfire) {
                            finalResult = (isZH ? "⚠️逆火: " : "⚠️Backfire: ") + finalResult;
                        }
                        return { text: finalResult, raw: result };
                    };

                    // 分別推演 場上無金餅乾(0) 與 場上有金餅乾(1) 的結果
                    const pred0 = getPrediction(0); 
                    const pred1 = getPrediction(1); 
                    this._cache.fthof0 = pred0.text;
                    this._cache.fthof1 = pred1.text;

                    // --- 推演 Stretch Time (賭徒的紅綠燈燈號) ---
                    // 這裡完全獨立，不影響上方的預測，只是為了改變 Gambler 按鈕顏色
                    Math.seedrandom(Game.seed + '/' + (M.spellsCastTotal + 1));
                    
                    let stretchFailChance = 0.2; 
                    if (buffAdept) stretchFailChance *= 0.1;
                    if (buffInept) stretchFailChance *= 5;
                    if (buffDiminish) stretchFailChance *= 0.1;
                    if (auraMult > 0) stretchFailChance += 0.1 * auraMult;
                    
                    let stretchBackfire = !(Math.random() < (1 - stretchFailChance));
                    
                    // 根據 pred0 的 raw 判斷是否為大獎 (因源碼改為全小寫，這裡一併適配)
                    if (pred0.raw === 'click frenzy' || pred0.raw === 'building special') {
                        if (!stretchBackfire) {
                            this._cache.gamblerColor = 'Green';
                        } else {
                            this._cache.gamblerColor = 'Yellow';
                        }
                    } else {
                        this._cache.gamblerColor = 'Red';
                    }

                } finally {
                    // 10. 歸還 RNG 控制權 (必定執行，保護遊戲原生亂數不崩潰)
                    Math.random = originalRandom;
                    if (typeof Math.seedrandom === 'function') Math.seedrandom();
                }
                
                this._cache.key = currentKey;
            }
        },
        ...
        GodzamokTactical: {
            ...
            buyAmmo: function() {
                ...
                if (obj &&[2, 3, 4].includes(obj.id)) {
                    ...
                    // 注意：BuildingLimit 是以 Name Key 儲存的
                    ...
                    if (Game.cookies >= cost) {
                        ...
                        // 智慧解鎖回歸 & UI 強制同步
                        if (Config.Flags.SpendingLocked && !Runtime.GodzamokState.wasSpendingLocked && Runtime.GodzamokTacticalState.status !== 'IDLE') {
                            UI.GardenProtection.toggle(false);
                            $('#chk-spending-lock').prop('checked', false);
                            if (UI.GardenProtection.updateEmbeddedState) UI.GardenProtection.updateEmbeddedState();
                            if (UI.updateEmbeddedState) UI.updateEmbeddedState();
                            Logger.success('Tactical', '已還原原始支出狀態 (解鎖)');
                        }
        ...
        SugarLump: {
            //[v9.4.0] 新增糖塊雷達運算引擎 (升級回傳物件包含動態提示詞)
            updateRadar: function() {
                if (typeof Game === 'undefined' || !Game.canLumps()) return { color: '#666', text: '未解鎖糖塊' };
                let remainingMs = (Game.lumpT + Game.lumpMatureAge) - Date.now();
                
                if (remainingMs <= 0) return { color: Config.Settings.LumpColorMature, text: '🟢 糖塊已成熟！可安全收割' };
                
                let mins = remainingMs / 60000;
                if (mins > Config.Settings.LumpTimeRed) return { color: Config.Settings.LumpColorRed, text: `🔴 成長中 (距成熟大於 ${Config.Settings.LumpTimeRed} 分鐘)` };
                if (mins > Config.Settings.LumpTimeYellow) return { color: Config.Settings.LumpColorYellow, text: `🟡 即將成熟 (距成熟大於 ${Config.Settings.LumpTimeYellow} 分鐘)` };
                return { color: Config.Settings.LumpColorBlue, text: `🔵 成熟倒數 (距成熟小於 ${Config.Settings.LumpTimeYellow} 分鐘)` };
            },
            update: function(now) {
        ...
        Buy: {
            update: function(now) {
                ...
                //[v9.2.0] Sniper 命中監控中：阻斷大型資金消耗，保護現場
                if (Runtime.GardenState && Runtime.GardenState.sniperActiveCoord !== null) {
                    return;
                }
        ...
        updateTitle: function() {
            ...
            // 格式:[🎯B2][✅SAFE][⚡ATTACK|15.5k|100,200] Cookie Clicker Ultimate v9.6.1
            document.title = `${sniperPrefix}[${signalStatus}][${signal}|${displayMult}x|${coords}] Cookie Clicker Ultimate v9.6.1`;
        },
    ...
    const Core = {
        ...
        init: function() {
            Logger.success('Core', 'Cookie Clicker Ultimate v9.6.1 Loading...');
            ...
            // Input Listener System (v8.8.8.1 / 9.6.0 Update)
            let lastInputTime = 0;
            let lastF11Time = 0;
            document.addEventListener('keydown', function(e) {
                ...
                const checkHotkey = (setting) => {
                    if (!setting || setting.trim() === '') return false;
                ...
                // Godzamok Macro (一鍵連擊)
                if (checkHotkey(Config.Settings.GodzamokMacroHotkey)) {
                    e.preventDefault();
                    e.stopPropagation();
                    
                    const now = Date.now();
                    // ADV 憲法約束：物理防火牆，阻斷 AHK 1~10ms 級別的狂轟猛炸，強制冷卻 150ms
                    if (now - lastF11Time < 150 || Runtime.GodzamokTacticalState.status !== 'IDLE') return;
                    lastF11Time = now;
                    
                    // 1. 強制觸發全賣
                    Logic.GodzamokTactical.fire(); 
                    
                    // 2. ADV 憲法約束：微非同步執行，給予遊戲引擎 100ms 結算 Buff 與 DOM
                    setTimeout(() => {
                        // 3. 清除 fire 產生的 10s reloadTimer
                        if (Runtime.GodzamokTacticalState.reloadTimer) {
                            clearTimeout(Runtime.GodzamokTacticalState.reloadTimer);
                            Runtime.GodzamokTacticalState.reloadTimer = null;
                        }
                ...
                // [v9.6.0] Gambler Spin Hotkey
                if (checkHotkey(Config.Settings.GamblerHotkey)) {
                    e.preventDefault();
                    e.stopPropagation();
                    Logic.Gambler.spin();
                    return;
                ...
                // [v9.6.0] Hand of Fate Hotkey
                if (checkHotkey(Config.Settings.HoFHotkey)) {
                    e.preventDefault();
                    e.stopPropagation();
                    const Tower = Game.Objects['Wizard tower'];
                    if (Tower && Tower.minigameLoaded) Tower.minigame.castSpell(Tower.minigame.spells['hand of fate']);
        ...
        startHeartbeat: function() {
            ...
            this.heartbeatTimer = setInterval(() => {
                ...
                //[v9.3.0/9.6.0] Update Magic Monitor
                ...
                //[v9.4.0] 糖塊紅綠燈 DOM 注入與自癒 (升級版 LED 實體燈號)
                $('#cc-lump-traffic-light').remove(); 
                
                const masterTarget = $('#buildingsMaster');
                if (masterTarget.length > 0 && $('#cc-lump-radar-btn').length === 0) {
                    // 建立 32x32 的外框，內部塞入一個 20x20 的實體發光 div (LED)
                    masterTarget.append(`
                        <div id="cc-lump-radar-btn" style="position: absolute; top: 10px; left: 170px; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; cursor: help; z-index: 10000; pointer-events: auto; background: rgba(0,0,0,0.5); border-radius: 4px; border: 1px solid #555; transition: all 0.3s; box-shadow: 0 2px 5px rgba(0,0,0,0.5);">
                            <div id="cc-lump-radar-indicator" style="width: 20px; height: 20px; border-radius: 50%; transition: all 0.5s;"></div>
                        </div>
                    `);
                }
                if ($('#cc-lump-radar-btn').length > 0) {
                    let radarInfo = Logic.SugarLump.updateRadar();
                    
                    // 針對內部的 LED 燈球上色並加上光暈
                    $('#cc-lump-radar-indicator').css({
                        'background-color': radarInfo.color,
                        'box-shadow': `0 0 10px ${radarInfo.color}, inset 0 0 4px rgba(255,255,255,0.6)`
                    });
                    
                    // 外部按鈕邊框與提示文字更新
                    $('#cc-lump-radar-btn')
                        .css('border-color', radarInfo.color)
                        .attr('title', radarInfo.text);
                }
                ...
                if (Config.Flags.PreRestartRecovery && Runtime.Timers.NextRestart > 0) {
                    ...
                    if (timeLeft <= Config.Settings.RecoveryTimeMs && !Runtime.Cache.hasRecovered) {
                        ...
                        $('#chk-spell, #chk-se').prop('checked', true);
                        Logic.EmergencyForceMutation(); // 暴力解鎖與換土
                        Runtime.Cache.hasRecovered = true;
                        Logger.success('Recovery', '防呆重啟協議已觸發：已強制解除鎖定並恢復掛機狀態');
                    }
                }
