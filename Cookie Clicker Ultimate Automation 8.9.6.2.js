...
// @description:zh-TW 全功能自動掛機腳本 v8.9.6.2 Fortune Cookie Ticker
...
/*
變更日誌 (Changelog):
v8.9.6.2 Security Update (2025):
  - [Logic] Season: 實裝 Season ID Hardening，增加靜態黑名單過濾季節開關，防止無限重買迴圈。
v8.9.6.1 UI Fix (2025):
  - [UI] Action Log: 修正日誌面板佈局，改用 Flexbox 確保列表區域隨高度自動縮放。
v8.9.6 UI & Feature Update (2025):
  - [UI] Action Log: 支援動態高度調整 (Resize) 與高度記憶。
  - [Feature] Stock: 新增股市「一鍵清倉」按鈕 (📉)，位於銀行標題列。
  - [Config] Buy: 預設購買策略改為 Smart (智慧回本)。
v8.9.5.2 Logic Update (2025):
  - [Logic] Garden: 針對新品種 (紫框) 收割邏輯增加 +1 tick 安全緩衝，防止浮點數邊界導致種子未掉落。
v8.9.5.1 Hotfix (2025):
  - [Logic] Season: 修正季節開關無限購買迴圈問題，購買清單已排除切換開關 (toggle)。
v8.9.5 Logic Update (2025):
  - [Feature] Fortune: 新增自動點擊幸運跑馬燈 (Fortune Cookie Ticker) 功能，自動獲取綠色新聞的升級獎勵。
v8.9.4.1 Hotfix (2025):
  - [Logic] Season: 實裝 Strict Count Protocol (嚴格計數協議)，將季節判斷邏輯改為正向計數，防止高負載下的狀態誤判。
  - [Data] Season: 靜態化復活節 ID 列表，防止 Array.from 在極端延遲下失效。
...
(function() {
    ...
    const Config = {
        ...
        Flags: {
            ...
            Fortune: GM_getValue('isFortuneEnabled', true), // [v8.9.5 New]
        ...
        Settings: {
            ...
            BuyStrategy: GM_getValue('buyStrategy', 'smart'), // [Mod] v8.9.6 預設改為 smart
        ...
        Memory: {
            ...
            ActionLogHeight: GM_getValue('actionLogHeight', 250), // [New] 預設 250px
        ...
        create: function() {
            ...
            this.Elements.Container = $(`
                <div id="action-log-panel" style="
                    ...
                    width: 400px;
                    height: ${Config.Memory.ActionLogHeight}px; /* [Mod] 改由 Config 控制 */
                    /* [v8.9.6.1 Fix] Flex Layout & Overflow Control */
                    resize: vertical;
                    overflow: hidden;
                    display: flex;
                    ...
                    /* [Mod] Initial State: Last rule wins, but jQuery fadeIn restores 'flex' if set inline */
                    display: none; 
                ">
                    ...
                    <div id="log-list" style="
                        /* [v8.9.6.1 Fix] Flex layout child */
                        flex: 1;
                        overflow-y: auto;
                        overflow-x: hidden;
                        padding: 5px;
            ...
            // [v8.9.6.1 Fix] Ensure flex display is used when shown
            // jQuery's fadeIn sets display to 'block' if hidden by 'display:none'.
            // To fix this, we set CSS in JS to ensure toggling works with flex.
    ...
    const Core = {
        ...
        init: function() {
            Logger.success('Core', 'Cookie Clicker Ultimate v8.9.6.2 Loading...');
