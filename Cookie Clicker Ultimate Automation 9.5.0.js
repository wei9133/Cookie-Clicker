...
// @description:zh-TW 全功能自動掛機腳本 v9.5.0 The Final Vanguard
...
createControlPanel: function() {
            ...
            this.Elements.Panel = $(`
                ...
                ">
                    ...
                    ">
                        <span>🍪 控制面板 v9.5.0 The Final Vanguard</span>
                            ...
                            <div class="panel-section" style="margin-bottom:15px; padding:10px; background:#e1f5fe; border-radius:8px;">
                                ...
                                <div style="margin-top:10px; padding-top:10px; border-top:1px dashed #bcd; display:block;">
                                    ...
                                    <div style="font-size:11px; color:#555; margin-left:20px; margin-top:6px; padding-top:6px; border-top:1px dotted #ccc;">
                                        ...
                                        補貨熱鍵: <input type="text" id="val-godzamok-buy-hotkey" value="${Config.Settings.GodzamokBuyHotkey}" style="width:60px;"> (預設 F10)<br>
                                        一鍵連擊: <input type="text" id="val-godzamok-macro-hotkey" value="${Config.Settings.GodzamokMacroHotkey}" style="width:60px;">
        ...
        bindEvents: function() {
            ...
            $('#val-godzamok-macro-hotkey').change(function() {
                var val = this.value.trim();
                if (!val) return;
                Config.Settings.GodzamokMacroHotkey = val;
                GM_setValue('godzamokMacroHotkey', val);
            });
...
// ═══════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════
    // [v9.4.0] 奧術戰術面板 (Grimoire Tactical HUD) 與 神諭系統
    // ═══════════════════════════════════════════════════════════════
    UI.MagicMonitor = {
        ...
        update: function() {
            ...
            //[v9.4.0] 呼叫 Oracle 更新預測
            if (Logic.Oracle && Logic.Oracle.predictFtHoF) {
                const predictions = Logic.Oracle.predictFtHoF();
                
                const formatPrediction = (result) => {
                    if (result === "Click Frenzy" || result === "Building Special") {
                        return `<span style="color:#00ffff; text-shadow:0 0 5px #00ffff;">${result}</span>`;
                    }
                    return result;
                };

                this.Elements.Oracle0.html(formatPrediction(predictions[0]));
                this.Elements.Oracle1.html(formatPrediction(predictions[1]));
            }
        },
...
//[v9.1.9] Create Sidebar HUD (Fix: Absolute Position, No Layout Shift)
    ...
    // ═══════════════════════════════════════════════════════════════
    // 2. 核心邏輯模組 (Logic Fix v9.2.1 Matrix Sniper + v9.4.0 Oracle)
    // ═══════════════════════════════════════════════════════════════
    const Logic = {
        //[v9.1.3] 系統狀態檢查
       ...
       // ═══════════════════════════════════════════════════════════════
        //[v9.4.0.1] 奧術神諭系統 (Oracle) Try-Finally 絕對安全還原版
        // ═══════════════════════════════════════════════════════════════
        Oracle: {
            predictFtHoF: function() {
                const Tower = Game.Objects['Wizard tower'];
                if (!Tower || !Tower.minigameLoaded) return ["未知", "未知"];
                const M = Tower.minigame;

                let predictions = [];
                
                // 模擬狀態：[0] 畫面上無金餅乾, [1] 畫面上已有1個金餅乾 (影響逆火率)
                for (let shimmerCount = 0; shimmerCount <= 1; shimmerCount++) {
                    let result = "";
                    let isBackfire = false;

                    // 🛡️ 絕對安全防線：Cookie Clicker 不支援 new Math.seedrandom。
                    // 必須使用 try-finally 保證全域亂數種子一定會被還原，防止污染。
                    try {
                        Math.seedrandom(Game.seed + '/' + M.spellsCastTotal);
                        
                        // 1. 計算逆火率 (包含金餅乾懲罰與天賦加成)
                        let failChance = 0.15;
                        if (Game.hasBuff('Magic adept')) failChance *= 0.1;
                        if (Game.hasBuff('Magic inept')) failChance *= 5;
                        if (Game.hasBuff('Diminish ineptitude')) failChance *= 0.1;
                        failChance += shimmerCount * 0.1;

                        isBackfire = Math.random() < failChance;

                        // 2. 抽取結果
                        let choices =[];
                        if (isBackfire) {
                            choices =['Ruin', 'Ruin', 'Clot', 'Clot', 'Cursed Finger', 'Blood Frenzy'];
                            if (Math.random() < 0.1) choices.push('Blab');
                        } else {
                            choices = ['Frenzy', 'Lucky'];
                            if (!Game.hasBuff('Dragonflight')) choices.push('Click Frenzy');
                            if (Math.random() < 0.1) choices.push('Cookie Storm', 'Cookie Storm', 'Blab');
                            if (Game.BuildingsOwned >= 10 && Math.random() < 0.25) choices.push('Building Special');
                            if (Math.random() < 0.15) choices = ['Free Sugar Lump'];
                        }
                        
                        result = choices[Math.floor(Math.random() * choices.length)];

                    } finally {
                        // 無論如何，重置全域隨機數回到以時間為基準的狀態
                        Math.seedrandom(); 
                    }
    ...
    const Core = {
        ...
        init: function() {
            Logger.success('Core', 'Cookie Clicker Ultimate v9.5.0 Loading...');
